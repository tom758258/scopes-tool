...
// Aggregate demo-query state key -> owning section command id + display label key.
const STATE_MAPPING = [
  ["enabled", "demo-output", "output"],
  ["function", "demo-function", "function"],
  ["phase_degrees", "demo-phase", "phase"],
];
...
export class DemoEditor {
  ...
  buildDom() {
    ...
    this.refreshButton.textContent = translate("demo.editor.read");
  ...
  selectedDefinition() {
    ...
    return selected?.editor === "demo" ? selected : null;
  }
  ...
  renderAggregateState(state) {
    for (const [resultKey, commandId, labelSuffix] of STATE_MAPPING) {
      ...
      label.textContent = translate(`demo.state.${labelSuffix}`);
  ...
  setterValueFor(entryId, payload) {
    ...
    switch (entryId) {
      case "demo-output": return payload?.output?.enabled;
      case "demo-function": return payload?.function?.function;
      case "demo-phase": return payload?.phase?.phase_degrees;
  ...
  renderSetterState(entry, payload) {
    ...
    row.className = "wgen-editor-state-row";
    const label = document.createElement("span");
    label.textContent = translate(`demo.state.${found[2]}`);
    const shown = document.createElement("span");
    shown.textContent = typeof value === "boolean"
      ? translate(value ? "enum.enable" : "enum.disable")
      : String(value);
