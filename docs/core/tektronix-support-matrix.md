...
- Tek IEEE status uses `*ESR?`; `ALLEv?`, `EVENT?`, `EVMsg?`, and `EVQty?` address an Event Queue rather than the current normalized system-error queue. Reads of status or events can clear them.
