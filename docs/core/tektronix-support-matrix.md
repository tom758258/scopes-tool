...
- **PARTIAL AGGREGATE**: an existing read-only projection can return useful fields while representing unavailable fields using its existing null/unknown/omitted-value convention. Unsupported queries must be skipped, not attempted and caught. Nullable fields do not relax the required hardware operation, result source, or raw-response provenance. This does not authorize partial setters, artifacts, measurements, or workflows.
...
Inputs must satisfy both public validation and documented hardware limits. Raw fields retain actual instrument responses from the operation required by the contract. Readback must describe the setting being queried, not a related setting. An option subset applies only to public surfaces that already accept and execute it; artifact encoding, filename, and adapter behavior remain part of the contract. Unrecognized or out-of-subset instrument state must not be normalized into a supported value. Supported actions still report instrument errors, unavailable waveforms, missing storage, and timeouts; support does not guarantee success in every instrument state.
...
| `measure-results` (native result dump) | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED | Existing Core/CLI/Worker contract reads `:MEASure:RESults?`, preserves that result response in raw, and parses it best-effort. Tek per-slot TYPE/SOURCE/UNITS/VALUE queries do not supply that result dump; concatenating them changes command/result provenance. `MEASUrement?` and `MEASUrement:MEAS<x>?` are settings queries, not documented equivalent result queries (B2 pp. 128-129, 135-141; B1 pp. 2-136, 2-144-2-150). |
...
| `screenshot` (Core/CLI explicit BMP host artifact); Core `capture_screenshot` | UNSUPPORTED | SUPPORTED | UNSUPPORTED | Existing Core options.format=bmp / CLI --format bmp only: HARDCopy START over USBTMC, format BMP, port USB, INKSaver (B1 pp. 2-96-2-102). No bmp8bit support. B2 file-save/readback lacks required background/ink-saver controls. TBS1052B BMP applicability conflicts; see ambiguities. Does not enable the PNG/WebUI surface below. |
| Core `capture_screenshot_png`; CLI default/explicit PNG; current WebUI screenshot | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED | PNG bytes and a PNG host artifact are required. Current WebUI accepts only background, calls capture_screenshot_png, and writes .png. TDS2024B hardcopy BMP cannot satisfy this path. B2 lacks the required background/ink-saver controls; B1 lacks PNG output. No transcoding, relabeling BMP as PNG, or adapter format change. |
...
| `save-image-format` | SUPPORTED | UNSUPPORTED | UNSUPPORTED | B2 PNG/BMP via SAVE:IMAGE:FILEFORMAT set/query (p. 157). B1 SAVE:IMAGE:FILEFORMAT has no query form (p. 2-169); HARDCopy:FORMat? is not established as readback of that same save-image setting (p. 2-98). A BMP setter alone does not satisfy the complete setting/query operation or WebUI set/readback. No JPG/bmp8/bmp24. |
...
TDS2024B explicit BMP capture receives the documented hardcopy stream through end-of-message, validates the BMP signature, and retains BMP host artifact path/format semantics. Preserve the temporary 10-second timeout and restore it. Restore temporary hardcopy format/port changes as well. HARDCopy:FORMat? can read back its own hardcopy setting; this does not make it a save-image-format readback. A background request reads INKSaver, temporarily selects black/white, and restores it even after failure. Explicit ink-saver and landscape/portrait layout options map to B1 controls; palette must be omitted because no independent palette control is documented. The [Core screenshot API](../../src/scopes_tool_core/screenshot.py) and [CLI capture path](../../src/scopes_tool_cli/commands/workflows.py) accept an explicit BMP format. The [WebUI catalog](../../src/scopes_tool_webui/command_catalog.py) exposes only background, and [WebUI execution](../../src/scopes_tool_webui/command_execution.py) fixes the helper, writer, and extension to PNG. Core PNG capture, CLI default/explicit PNG, and current WebUI screenshot remain unsupported.

For `save-image`, use the caller's validated instrument filename/path and current save settings, subject to storage/media preconditions. The action neither requires nor returns a particular encoding; a current Tek JPEG setting does not add JPEG to the public format-setter enum. Report completion only after successful *OPC? under the existing temporary 15-second timeout, restored in finally. B2 table 30 and B1 table 3-3 explicitly list file saves as operation-complete operations. The common save-image contract requires no additional RUI/operation-condition polling.

The [Core save-format setter](../../src/scopes_tool_core/save_export.py) and CLI set action do not inherently perform immediate format readback. However, the existing operation also includes query_save_image_format / --query, returning the actual instrument-side format and raw_response; WebUI settings execute the setter followed by that getter. The complete row therefore requires both a setter and a matching readback. B1 documents SAVE:IMAGE:FILEFORMAT and HARDCopy:FORMat separately; its factory/setup listing includes both fields (pp. B-1-B-2). Their overlapping USB-image use does not establish that HARDCopy:FORMat? reads the SAVE:IMAGE:FILEFORMAT state. Do not use the other setting, an echoed argument, or a factory default as verification. The set-only primitive does not enable this operation.
...
MeasurementResultsDump is excluded from these projections. The [Core results query/parser](../../src/scopes_tool_core/measurements.py) preserves one native result response, while the CLI contract also identifies its command as `:MEASure:RESults?`. Best-effort parsing and nullable statistics apply to that response; they do not authorize synthesizing raw from multiple periodic-slot operations. Tek `MEASUrement?` / `MEASUrement:MEAS<x>?` return settings; the B2 example's statistics fields do not establish an equivalent current-results dump. Reading TYPE/SOURCE/UNITS/VALUE separately would change the operation and raw-result semantics even without instrument mutation. All three models therefore remain unsupported for measure-results.
