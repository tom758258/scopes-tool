...
`Infinity`. Public Core API entry points (`plan_sequence()`, `run_sequence()`)
strictly validate and normalize inputs before hardware access, directory
creation, or manifest initialization.
...
- `wait-trigger`: requires positive finite `timeout_seconds` and waits for an
  ...
  single-shot flow. Cancellation observed during trigger polling exits
  immediately without sending additional system-error queries.
...
- `screenshot`: captures the existing cross-series PNG form and accepts an
  optional `background` of `black` or `white`. Dry-run planning for screenshot
  lists guaranteed static queries (`:HARDcopy:INKSaver?`, `:DISPlay:DATA? PNG, COLor`,
  `:SYSTem:ERRor?`) without asserting conditional runtime background writes.
...
the manifest holds the repeated execution history. If a step fails during
artifact creation or manifest update, existing deterministic files are
preserved in partial results, but incomplete steps are not counted as
completed and do not trigger progress callbacks.

Cooperative cancellation is checked at entry, before steps, after persisted
successful steps, at loop boundaries, and during host and trigger polling
waits. Pre-start cancellation performs zero hardware I/O and creates no run
directory (`output_dir: null`, `manifest_path: null`, `scpi_log_path: null`).
Completed steps and artifacts remain valid. `completed` has precedence over a
stop request observed after all finite work; system or step failure has
precedence over cancellation. Cooperative cancellation returns `status: "cancelled"`,
