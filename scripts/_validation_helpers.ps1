...
$script:ValidationTargetProfiles = @(
    [pscustomobject]@{ model_id = "keysight-dsox2004a"; model = "DSO-X 2004A" },
    [pscustomobject]@{ model_id = "keysight-dsox3024a"; model = "DSO-X 3024A" },
    [pscustomobject]@{ model_id = "keysight-dsox4024a"; model = "DSO-X 4024A" },
...
function Get-ValidationTargetProfiles {
    ...
    foreach ($profile in @($script:ValidationTargetProfiles)) {
        ...
        if ([string]::IsNullOrWhiteSpace([string]$profile.model)) {
...
function New-ValidationRunDirectory {
    ...
    $root = Join-Path (Get-FullPath -Path $BaseRoot) ("${Prefix}_" + (Get-Date -Format "yyyyMMdd_HHmmss_fff"))
    $private = Join-Path $root "private"
    $shareable = Join-Path $root "shareable"
    New-Item -ItemType Directory -Force -Path $private, $shareable | Out-Null
...
function Invoke-CapturedCommand {
    ...
    try {
        if ((Test-Path -LiteralPath $StdErrPath) -and
            [string]::IsNullOrWhiteSpace((Get-Content -LiteralPath $StdErrPath -Raw))) {
    ...
    return [ordered]@{
        ...
        stdout = $StdOutPath
        stderr = $stderrRecorded
        json = $jsonPathWritten
    }
}
