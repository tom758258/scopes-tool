...
function ConvertTo-ShareableArtifactValue {
    ...
    if ($key -in @('idn', 'raw_idn', 'idn.raw')) { return '<redacted-idn>' }
    ...
    if ($Value -is [System.Collections.IDictionary]) {
        ...
        foreach ($entryKey in $Value.Keys) {
            $result[[string]$entryKey] = ConvertTo-ShareableArtifactValue -Value $Value[$entryKey] -FieldName ([string]$entryKey) -PrivateRoot $PrivateRoot -ShareableRoot $ShareableRoot -RepoRoot $RepoRoot -Resource $Resource -SensitiveValues $SensitiveValues
        }
        return $result
    }
    if ($Value -is [pscustomobject]) {
        ...
        foreach ($property in $Value.PSObject.Properties) {
            $result[$property.Name] = ConvertTo-ShareableArtifactValue -Value $property.Value -FieldName $property.Name -PrivateRoot $PrivateRoot -ShareableRoot $ShareableRoot -RepoRoot $RepoRoot -Resource $Resource -SensitiveValues $SensitiveValues
        }
        return $result
    }
    if ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) {
        $items = @($Value | ForEach-Object { ConvertTo-ShareableArtifactValue -Value $_ -FieldName $FieldName -PrivateRoot $PrivateRoot -ShareableRoot $ShareableRoot -RepoRoot $RepoRoot -Resource $Resource -SensitiveValues $SensitiveValues })
        return ,$items
    }
    return $Value
}
...
function Convert-PrivateJsonArtifact {
    ...
    if (-not (Test-Path -LiteralPath $SourcePath)) {
        ...
        try {
            ...
            $payload = ConvertTo-ShareableArtifactValue -Value $parsed -PrivateRoot $Context.PrivateRoot -ShareableRoot $Context.ShareableRoot -RepoRoot $Context.RepoRoot -Resource $Context.Resource -SensitiveValues $Context.SensitiveValues
        } catch {
...
function New-ShareableArtifactSet {
    ...
    $context = @{
        ...
        ShareableRoot = (Get-FullPath -Path $ShareableRoot)
        RepoRoot = (Get-FullPath -Path $RepoRoot)
        Resource = [string]$Resource
        SensitiveValues = @(
            @(Get-DistinctiveSensitiveTokens -Resource ([string]$Resource)) + @($ExtraSensitiveValues)
        )
    }
    Copy-ShareableArtifactTree -Context $context

    $report = ConvertTo-ShareableArtifactValue -Value $PrivateReport -PrivateRoot $context.PrivateRoot -ShareableRoot $context.ShareableRoot -RepoRoot $context.RepoRoot -Resource $context.Resource -SensitiveValues $context.SensitiveValues
    ...
    Write-Utf8NoBomText -LiteralPath (Join-Path $context.ShareableRoot 'report.json') -Text ($report | ConvertTo-Json -Depth 20)

    $summarySafe = ''
    if (Test-Path -LiteralPath $PrivateSummaryPath) {
    ...
    Write-Utf8NoBomText -LiteralPath (Join-Path $context.ShareableRoot 'summary.md') -Text $summarySafe
    return $report
}
