...
if ($normalizedConnection -notin @("usb", "tcpip")) {
    Write-LiveUsageError -Domain "dvm" `
...
Write-Host "[live][dvm] target: $($script:Target)"
Write-Host "[live][dvm] connection: $($script:Connection)"
Write-Host "[live][dvm] artifacts: $($script:RunDirectory)"
Write-Host ""
...
Write-Host "Summary"
foreach ($entry in $script:CaseResults.GetEnumerator()) {
    Write-Host ("{0,-5} [live][dvm] {1}" -f $entry.Value.Status, $entry.Key)
}
Write-Host "[live][dvm] artifacts: $($script:RunDirectory)"
Write-Host ""
...
if ($script:FunctionalFailed) {
    ...
    if ($script:ShareableGenerationFailed) {
        Write-Host "[live][dvm] run failed; see private report for the shareable generation error"
    }
    exit 1
}

if ($script:DvmUnavailable) {
    Complete-LiveValidationRun -Kind 'scopes-tool-live-dvm-check' -Domain 'dvm' -Result "SKIP"
    Write-Host "SKIP  DVM live validation"
    Write-Host "      DVM option/license is not available on this instrument."
    if ($script:ShareableGenerationFailed) {
        Write-Host "[live][dvm] skipped run failed; see private report for the shareable generation error"
        exit 1
    }
    exit 0
}
