...
working on Scopes Tool. Global agent rules already cover communication,
...
- The root `pyproject.toml` is the single distribution metadata boundary for
  `scopes-tool`. Do not recreate component-local distributions or introduce a
  `scopes_tool.*` namespace without explicit user approval.
...
- Preserve the import packages `scopes_tool_core`, `scopes_tool_cli`, and
  `scopes_tool_webui`.
...
- Keep model identification, capabilities, drivers, validation, and
  ...
  `*IDN?`; `--model` must not override it. Unknown, unregistered, or mismatched
  vendors, models, profiles, or drivers fail closed. Dry-run and simulator
  paths may use an explicitly selected registered profile.
...
- Treat changes that can affect a live oscilloscope as high risk. Get user
  confirmation before changing SCPI behavior, VISA timeout defaults,
  trigger/wait strategy, acquisition mode, waveform defaults, return-to-local
  behavior, or cleanup that can leave the instrument stopped, locked, or with
  changed front-panel state.
- Do not use `*RST`, preset, autoscale, setup recall, or other broad
  state-changing operations by default. Keep SCPI generation and model-specific
  query, wait, parsing, and safety behavior in Core; do not generalize them
  without model-appropriate validation and explicit approval.
- Real hardware access is explicit and opt-in. One-shot live access requires a
  user-supplied `--resource` or `SCOPES_TOOL_RESOURCE`; live Workers require
  both `--live` and `--resource`. Do not infer, guess, or scan for resources;
  only `list-resources --live-only` may open enumerated resources for discovery.
  Never commit real resources, serial numbers, or private lab addresses.
